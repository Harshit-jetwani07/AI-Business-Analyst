...
RUNTIME_RETENTION_DAYS = int(os.getenv("RUNTIME_RETENTION_DAYS", "30"))
...
def cleanup_runtime_files():
    if os.getenv("ENABLE_RUNTIME_CLEANUP", "").lower() not in {"1", "true", "yes"}:
        return
    cutoff = time.time() - (RUNTIME_RETENTION_DAYS * 24 * 60 * 60)
    for folder in ["uploads", "reports", "exports"]:
        path = Path(folder)
        if not path.exists():
            continue
...
def super_smart_data_loader(uploaded_file):
    ...
    try:
        ...
        if raw_df.shape[0] > MAX_DATASET_ROWS or raw_df.shape[1] > MAX_DATASET_COLS:
            st.error(f"Dataset limit exceeded. Maximum allowed shape is {MAX_DATASET_ROWS:,} rows x {MAX_DATASET_COLS} columns.")
            return None
        ...
        for col in df.columns:
            ...
            try:
                sanitized_series = df[col_clean].astype(str).apply(clean_numeric_text)
...
def refresh_current_dataset_status():
    dataset_id = st.session_state.get("dataset_id")
    if not dataset_id:
        return
    record = get_dataset_record(dataset_id)
    if record:
        st.session_state.dataset_status = record.get("status") or "pending"
        st.session_state.quality_report = st.session_state.get("quality_report") or {
            "score": record.get("quality_score", 0),
            "grade": record.get("quality_grade", ""),
            "duplicate_rows": 0,
            "duplicate_pct": 0,
            "issues": [],
        }
...
with st.sidebar:
    ...
    api_key_fingerprint = hashlib.sha256(api_key.encode("utf-8")).hexdigest() if api_key else None
    if api_key and st.session_state.api_key_fingerprint != api_key_fingerprint:
        try:
            ...
            st.error(str(e))
    elif not api_key and st.session_state.api_key_set:
    ...
    if st.session_state.api_key_set:
        ...
        st.session_state.ai_data_consent = st.checkbox(
            "Allow AI features to send masked dataset summaries/sample rows to this provider",
            value=st.session_state.get("ai_data_consent", False),
        )
    else:
...
if st.session_state.df is None:
    ...
    refresh_current_dataset_status()
    ...
    with tab3:
        ...
        elif not st.session_state.get("ai_data_consent", False):
            st.warning("Enable AI data sharing consent in the sidebar to use chat. Sensitive fields are masked before sending.")
        else:
   ...
   #  TAB 4: FORECAST 
    ...
    with tab6:
        ...
        if not st.session_state.api_key_set:
            ...
            st.warning("Enable AI data sharing consent in the sidebar to generate reports. Sensitive fields are masked before sending.")

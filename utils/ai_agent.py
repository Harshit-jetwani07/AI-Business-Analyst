...
SENSITIVE_COLUMN_HINTS = {
    "email", "phone", "mobile", "contact", "address", "ssn", "aadhaar",
    "pan", "password", "token", "secret", "api_key", "apikey", "card"
}
...
def mask_sensitive_text(value):
    ...
    text = re.sub(
        r"\+?[\d][\d\s().-]{7,}[\d]",
        lambda m: "[PHONE]" if len(re.sub(r"\D", "", m.group(0))) >= 10 else m.group(0),
        text,
    )
...
def sanitized_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    safe_df = df.copy()
    for col in safe_df.columns:
        col_l = str(col).lower()
        if any(hint in col_l for hint in SENSITIVE_COLUMN_HINTS):
            safe_df[col] = "[REDACTED]"
        elif pd.api.types.is_string_dtype(safe_df[col]) or safe_df[col].dtype == "object":
            safe_df[col] = safe_df[col].map(mask_sensitive_text)
    return safe_df
...
class AIAgent:
    ...
    def answer_question(self, df: pd.DataFrame, question: str, chat_history: list) -> str:
        try:
            safe_df = sanitized_dataframe(df)
            numeric_summary = safe_df.select_dtypes(include=[np.number]).describe().round(2).to_string()
            missing_summary = safe_df.isnull().sum().to_string()
            sample_rows = safe_df.head(8).to_string(index=False)
            data_summary = (
